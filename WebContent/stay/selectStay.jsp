...
<%@ page language="java" contentType="text/html; charset=UTF-8"
   ...
   <%
		...
		List<StayVO> list  = (List<StayVO>)request.getAttribute("stayAllList");
...
<head>
  ...
  <link rel="stylesheet" href="/FireMoth/stay/selectStay_css.css">
				...
				<div id="content_box">
					<div id="content_box_container">
						...
						<div class="s_container">
							...
							%>
								<div onclick="location='detailStay.do?stay_id=<%=list.get(i).getStay_id()%>'" class="s_box">
									<div class="stay_img_div">
										
									<%
										if(atchList.get(i) != null){
									%>			
												<img  class="stay_img" src="<%= request.getContextPath() %>/filedownload.do?fileId=<%= atchList.get(i).getAtchFileId() %>&fileSn=<%= atchList.get(i).getFileSn() %>">
									<%			
										}
									...
									<div class="stay_info_div">
										<h2><%=list.get(i).getStay_name()%></h2><br>
										<p class="p">숙소 상품번호 : <%=list.get(i).getHost_no()%></p>
										<p class="p">최대 수용인원 : <%=list.get(i).getStay_pnum()%></p>
										<p class="p">가격 : <%=list.get(i).getStay_price()%></p>
										<p class="p">주소 : <%=list.get(i).getStay_add()%></p>
									</div>
								</div>
							<%
										}
							...
							<br>
							<button onclick="location='stay/insertStay.jsp'" class="base_btn">등록</button>
